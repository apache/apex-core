...
public class Connector implements AutoCloseable
{
  ...
  private ConcurrentLinkedQueue<Event> concurrentEvents = new ConcurrentLinkedQueue<Event>();
  ...
  public void replace(Connection oldConnection, int ops, Connection newConnection)
  {
    newConnection.events = oldConnection.events;
    newConnection.concurrentEvents = oldConnection.concurrentEvents;
    newConnection.socketChannel = oldConnection.socketChannel;
    newConnection.selectionKey = oldConnection.selectionKey;
    newConnection.selectionKey.attach(newConnection);
    if (oldConnection instanceof Listener) {
      listeners.remove((Listener)oldConnection);
    }
    if (newConnection instanceof Listener) {
      listeners.add((Listener)newConnection);
    }

    newConnection.appendFilters(filterFactories);
  }
  ...
  public boolean doEvents()
  {
    ...
    for (SelectionKey key : selectedKeys) {
      ...
      try {
        if (key.isReadable()) {
          int bytesRead = connection.socketChannel.read(connection.getBuffer());
          if (bytesRead > 0) {
            connection.onRecv(bytesRead);
          }
          else if (bytesRead < 0) {
