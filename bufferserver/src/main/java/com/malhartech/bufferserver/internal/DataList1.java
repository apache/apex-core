...
{
//  private static final Logger logger = LoggerFactory.getLogger(DataList1.class);
...
//      long ending_window;
//      if (temp.resets.isEmpty()) {
//        starting_window = 0;
//        ending_window = 0;
//      }
//      else {
//        starting_window = temp.resets.get(0).windowId;
//        ending_window = temp.resets.get(temp.resets.size() - 1).windowId;
//      }
//      if (starting_window >= longWindowId || ending_window > longWindowId) {
//        if (temp != last) {
//          temp.next = null;
//          last = temp;
//        }
...
//
//    DataArray prev = null;
//    for (DataArray temp = first; temp != null && (temp.resets.isEmpty() ? 0 : temp.resets.get(0).windowId) <= longWindowId; temp = temp.next) {
//      long ending_window = temp.resets.isEmpty() ? 0 : temp.resets.get(temp.resets.size() - 1).windowId;
//      if (ending_window > longWindowId || temp == last) {
//        if (prev != null) {
//          first = temp;
//        }
//
...
//    }
//
//    ArrayList<ResetTupleTracker> resets = new ArrayList<ResetTupleTracker>();
//    /**
...
//    {
//      long bs = resets.isEmpty() ? 0 : resets.get(0).windowId & 0x7fffffff00000000L;
...
//    {
//      logger.debug("starting_window = {}, longWindowId = {}", new Object[] {Codec.getStringWindowId(resets.isEmpty() ? 0 : resets.get(0).windowId), Codec.getStringWindowId(longWindowId)});
//      boolean found = false;
//      long bs = resets.isEmpty() ? 0 : resets.get(0).windowId;
...
//                }
//
//                ResetTupleTracker rtt = new ResetTupleTracker();
//                rtt.index = sd.offset;
//                rtt.windowId = bs | di.getWindowId(sd);
//
//                if (resets.isEmpty()) {
//                  resets.add(rtt);
//                }
//                else {
//                  resets.set(0, rtt);
//                  Iterator<ResetTupleTracker> iterator = resets.iterator();
//                  iterator.next();
//                  while (iterator.hasNext()) {
//                    ResetTupleTracker temprtt = iterator.next();
//                    if (temprtt.index <= sd.offset) {
//                      iterator.remove();
//                    }
//                  }
//                }
...
//  }
//
//  public DataList1(String identifier, int blocksize, int maxlivebocks)
//  {
...
//  }
//
//  public DataList1(String identifier)
//  {
//    /*
