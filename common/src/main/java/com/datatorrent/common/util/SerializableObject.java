...
public class SerializableObject implements Serializable
{
  ...
  public Object readResolve() throws ObjectStreamException
  {
    try {
      ...
      logger.debug("No copy constructor detected for class {}, trying default constructor.",
        ...
        if (!(Modifier.isFinal(modifiers) && Modifier.isTransient(modifiers) || Modifier.isStatic(modifiers))) {
          ...
          if (!field.getType().isPrimitive()) {
            try {
              ...
              logger.warn("Failed to set field {} to null; generally it's harmless, but with reference counted data " +
                  "structure this may be an issue.", field, ex);
            }
          }
        }
      }
